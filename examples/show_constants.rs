...
async fn main() -> Result<(), mycelium::Error> {
    ...
    let existential_deposit = api.fetch_constant_opaque_value("Balances", "ExistentialDeposit")?;
    ...
    let not_found = api.fetch_constant_opaque_value("Balances", "ThisDoesNotExists")?;
    let not_found: Result<u32, _> = codec::Decode::decode(&mut not_found.as_slice());
    dbg!(not_found);

    let max_comments = api.fetch_constant_opaque_value("ForumModule", "MaxComments")?;
    let max_comments: Result<u32, _> = codec::Decode::decode(&mut max_comments.as_slice());
    dbg!(max_comments);

    let max_content_length = api.fetch_constant_opaque_value("ForumModule", "MaxContentLength")?;
