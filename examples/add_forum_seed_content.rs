...
async fn main() -> anyhow::Result<()> {
    let entries: Vec<(&str,Vec<(&str, Vec<&str>)>)> = vec![
        ...
        ("Thou TCP/IP ensures the delivery and acknowledge, but UDP sacrifice accuracy for speed for applications such as games and movies, users don't want to wait\
        \n-- Sun Tzu, 1337 AD",
        vec![
        ]),
